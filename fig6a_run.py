...
from utils import get_random_regression_task, get_lambda_balanced, get_ntk,kernel_distance,BlindColours, kernel_distance
...
lmda_values = np.linspace(-9, 9, 40)
scale_values = np.linspace(1, 20, 40)
...
for lmda in lmda_values:
    for scale in scale_values:
        init_w1, init_w2 = get_lambda_balanced(lmda, in_dim, hidden_dim, out_dim, scale=scale)
...
lmda_ticks_labels = [-9, 0, 9]
scale_ticks_labels = [0, 10, 20]

# Define steps of interest
steps_of_interest = [4000]
...
# Plot kernel distance and loss heatmaps for each step
for i, step in enumerate(steps_of_interest):
    ...
    sns.heatmap(kernel_heatmaps[step], 
                ...
                square=True)
    np.save('./data/imba_heatmap',kernel_heatmaps[step])
    axes[i].set_title(f'Kernel Distance, t={step}', fontsize=10)
    axes[i].set_xlabel('Absolute Scale', fontsize=8)
    axes[i].set_ylabel('Relative Scale (λ)', fontsize=8)
    axes[i].set_xticks(x_tick_positions)  # Custom x-tick positions
    axes[i].set_xticklabels(x_tick_labels, fontsize=8)  # Custom x-tick labels
    axes[i].set_yticks(y_tick_positions)  # Custom y-tick positions
    axes[i].set_yticklabels(y_tick_labels, fontsize=8)  # Custom y-tick labels
